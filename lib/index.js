...
var defaultOptions = {
    discoveryUrlPrefix: module.exports.swaggerPathPrefix
};
...
module.exports.findOrCreateResource = function (resource, version, options) {
    assert.ok(swagger.resources, 'Swagger not initialized! Execution of configure required!');

    var found = _.find(swagger.resources[version], function (myResource) {
    ...
    var docs = found || swagger.createResource(resource, version, options || { models: {}, description: getApiDescription(resource) });
    return docs;
};
...
module.exports.loadRestifyRoutes = function () {
    ...
    _.each(this.server.router.mounts, function (item) {
        ...
        var versions = item.versions || ['1.0.0'];
        ...
        if (validationModel) {
            ...
            if (!_.contains(self.options.blacklist, mySwaggerPathParts)) {
                _.each(versions, function(version) {
                    var mySwaggerPath = module.exports.swaggerPathPrefix + version + '/' + mySwaggerPathParts;
                    var swaggerDoc = self.findOrCreateResource(mySwaggerPath, version, { models: models, description: getApiDescription(mySwaggerPath) });
                    var parameters = [];
                    var modelName = name;
                    ...
                    _.each(validationModel, function(model, scope) {
                        _.each(model, function (valueArray, key) {
                    ...
                    // avoid duplicated loadRestifyRoutes
                    if (!defined[spec.method+version+spec.url]) {
                        ...
                        defined[spec.method+version+spec.url] = true;
                    }
                });
            }
        }
